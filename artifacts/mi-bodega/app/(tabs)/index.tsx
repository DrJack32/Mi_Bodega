...
const COLLECTION_OPTIONS = [
  ...
  { label: "En bodega", value: "cellar" },
  { label: "Bebidos", value: "tasted" },
] as const;
...
export default function BibliotecaScreen() {
  ...
  const [collectionFilter, setCollectionFilter] = useState<
    "all" | "cellar" | "tasted"
  >("all");
  const [sortBy, setSortBy] = useState<"recent" | "rating" | "price" | "name">(
    "recent",
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (collectionFilter === "cellar") {
      list = list.filter((w) => getWineStockCount(w) > 0);
    } else if (collectionFilter === "tasted") {
      list = list.filter((w) => getWineTastedBottleCount(w) > 0);
    }

    if (filterType !== "all") {
      list = list.filter((w) => {
        if (filterType === "otro")
          return !["tinto", "blanco", "rosado", "espumoso"].includes(w.type);
    ...
    list.sort((a, b) => {
      switch (sortBy) {
        case "rating":
          return getWineAverageRating(b) - getWineAverageRating(a);
        case "price":
          return normalizePriceNumber(b.price) - normalizePriceNumber(a.price);
        case "name":
    ...
    return list;
  }, [wines, search, filterType, collectionFilter, sortBy]);

  const cellarBottleCount = useMemo(
    () => wines.reduce((sum, wine) => sum + getWineStockCount(wine), 0),
    [wines],
  );
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const active = collectionFilter === item.value;
            return (
              <Pressable
                onPress={() => setCollectionFilter(item.value)}
                style={[
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.foreground }]}>
              {search || filterType !== "all" || collectionFilter !== "all"
                ? "Sin resultados"
      ...
      <Pressable
        ...
        style={[
          styles.fab,
          {
            ...
            borderRadius: 30,
            bottom: bottomPad + (Platform.OS === "web" ? 84 : 80),
          },
        ]}
      >
...
const styles = StyleSheet.create({
  ...
  collectionChipText: { fontSize: 13, fontFamily: "Inter_700Bold" },
