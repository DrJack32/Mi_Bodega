...
export function WineForm({
  ...
  const runOCROnPhoto = async (uri: string, index: number) => {
    if (Platform.OS !== "android") {
      Alert.alert(
        ...
        "La lectura local de etiquetas esta disponible en la APK de Android.",
      );
      return;
    }
  ...
  const handleSave = async () => {
    if (!form.name.trim() && !form.winery.trim()) {
      Alert.alert(
        "Campos requeridos",
        "Por favor añade al menos el nombre del vino o la bodega.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "No se pudo guardar",
        `${message}\n\nEl formulario sigue abierto para que puedas intentarlo de nuevo.`,
      );
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <Pressable
          ...
          style={[
            styles.saveBtn,
            {
              ...
              borderRadius: colors.radius / 1.5,
              opacity: isSaving || isOcrLoading ? 0.5 : 1,
            },
          ]}
        >
          {isSaving ? (
      ...
      {isOcrLoading && (
        <View
          style={[styles.ocrProgressBar, { backgroundColor: colors.primary }]}
        >
          <ActivityIndicator size="small" color="#FFF" />
          <Text style={styles.ocrProgressText}>
            Analizando etiqueta con OCR…
          </Text>
        </View>
      )}
      ...
      >
        ...
        >
          ...
          <Text style={[styles.addPhotoLabel, { color: colors.foreground }]}>
            Añadir foto de la etiqueta
          </Text>
          <Ionicons
          ...
          {WINE_TYPES.map(({ value, label, color }) => (
            <Pressable
              ...
              style={[
                ...
                {
                  backgroundColor:
                    form.type === value ? color : colors.secondary,
                  borderRadius: 20,
            ...
            >
              <Text
                style={[
                  ...
                  {
                    color:
                      form.type === value ? "#FFF" : colors.mutedForeground,
                  },
                ]}
              >
                {label}
