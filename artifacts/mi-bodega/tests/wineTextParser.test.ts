...
test("reconoce ValdeOliva con añada romana e indicación geográfica", () => {
  const fields = parseWineText(`ValdeOliva
Roble
MMXXIV
Vino de la Tierra de Castilla y León
Tempranillo
Elaborado y embotellado por Bodegas Covitoro R.E.: 5326-ZA`);

  assert.equal(fields.name, "ValdeOliva");
  assert.equal(fields.winery, "Bodegas Covitoro");
  assert.equal(fields.vintage, "2024");
  assert.equal(fields.agingCategory, "Roble");
  assert.equal(fields.denomination, "IGP Vino de la Tierra de Castilla y León");
  assert.equal(fields.region, "Castilla y León");
  assert.equal(fields.country, "España");
  assert.equal(fields.grapes, "Tempranillo");
});
